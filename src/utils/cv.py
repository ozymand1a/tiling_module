...
def read_image_as_pil(
    image: Image.Image | str | np.ndarray, exif_fix: bool = True
) -> Image.Image:
    ...
    if isinstance(image, Image.Image):
        ...
        try:
            image_pil = Image.open(
                BytesIO(requests.get(image, stream=True).content)
            ...
            logger.error(
                f"PIL failed reading image with error {e}, trying skimage instead"
            )
            try:
                import skimage.io
        ...
        if image.ndim == 3 and image.shape[0] < 5 and image.shape[2] > 4:
